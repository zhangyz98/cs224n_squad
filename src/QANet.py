'''This file is the original file in repo
https://github.com/BangLiu/QANet-PyTorch/blob/master/model/QANet.py
This is for reference only and is not used in our training.
All used layers are moved to `sample_layers.py`.
'''
...
class QANet(nn.Module):
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        c_mask = torch.zeros_like(Cwid) != Cwid
        q_mask = torch.zeros_like(Qwid) != Qwid
        Cw, Cc = self.word_emb(Cwid), self.char_emb(Ccid)
        Qw, Qc = self.word_emb(Qwid), self.char_emb(Qcid)
        C, Q = self.emb(Cc, Cw, self.Lc), self.emb(Qc, Qw, self.Lq)
        Ce = self.emb_enc(C, c_mask, 1, 1)
        Qe = self.emb_enc(Q, q_mask, 1, 1)
